...
import { PRINTER_WIDTH, DEFAULT_COMPOSE_FONT_SIZE } from "@yhk/shared/constants";
import { ditherImageData, thresholdImageData } from "@yhk/shared/dither";
import { buildPrintJob } from "@yhk/shared/escpos";
import type { PrintBlock, QrErrorCorrection } from "@yhk/shared/print-document";
import {
  decodeBase64Image,
  fetchImageBytes,
} from "./image-loader.js";

export type { PrintBlock, QrErrorCorrection } from "@yhk/shared/print-document";
...
interface LayoutItem {
  kind: "title" | "text" | "qr" | "spacer" | "image";
  height: number;
  text?: string;
  font?: "normal" | "mono";
  maxLines?: number;
  ...
  spacerPx?: number;
  imageBlock?: Extract<PrintBlock, { type: "image" }>;
}
    ...
    for (const word of words.slice(1)) {
      ...
      if (context.measureText(candidate).width <= maxWidth) {
        ...
        if (maxLines !== undefined && lines.length >= maxLines) {
          return truncateLines(lines, maxLines, context, maxWidth);
        }
      }
    }
...
async function measureImageHeight(
  block: Extract<PrintBlock, { type: "image" }>,
  targetWidth: number,
): Promise<number> {
  const image = await loadImageBlock(block);
  const scale = targetWidth / image.width;
  return Math.max(1, Math.round(image.height * scale));
}
...
async function loadImageBlock(
  ...
  if (block.base64) {
    return loadImage(decodeBase64Image(block.base64));
  }

  throw new Error("Image block must include url or base64.");
}
  ...
  for (const block of blocks) {
    switch (block.type) {
      ...
      case "spacer":
        ...
        break;
      case "image": {
        const imageHeight = await measureImageHeight(block, width - PADDING * 2);
        layout.push({
          kind: "image",
          height: imageHeight,
          imageBlock: block,
...
export async function compose(
  ...
  const layout = await buildLayout(measureContext, blocks, width, typography);
  ...
  for (const item of layout) {
    switch (item.kind) {
      ...
      case "image": {
        if (!item.imageBlock) {
          throw new Error("Image layout item is missing block data.");
        }
        await drawImageBlock(context, item.imageBlock, PADDING, y, imageWidth);
        y += item.height;
        break;
      }
    }
  ...
  const hasDitheredImage = blocks.some(
    (block) => block.type === "image" && block.dither,
  );
  const pixels = hasDitheredImage
    ? ditherImageData(imageData.data, width, height)
    : thresholdImageData(imageData.data, width, height);

  return { pixels, height };
}

export async function composeToJob(
  blocks: PrintBlock[],
  options: ComposeOptions = {},
): Promise<{ job: Uint8Array; height: number }> {
  const { pixels, height } = await compose(blocks, options);
  return {
    job: buildPrintJob(pixels),
    height,
  };
}
